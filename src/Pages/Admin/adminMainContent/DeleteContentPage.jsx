    ...
    <>
        ...
        <>
            ...
            <section className={`select-none w-[800px] h-[300px] duration-400 rounded-md border-2 ${DarkMode ? 'border-white':'border-black'}`}>
                ...
                <form onSubmit={HandleSubmit} className={`${DarkMode ? 'text-white':'text-black'} w-full h-[calc(100%-18%)] flex flex-col gap-6 rounded-b-md px-6 pt-6`}>
                    <label htmlFor="projectID" className={`text-[22px] duration-400 font-extrabold quicksand-shit`}>Project ID:</label>
                    <input onChange={(e)=>{setprojectID(e.target.value);}} id="projectID" value={projectID} name="projectID" type="text" className={` duration-400 ${DarkMode ? 'border-white text-white':'border-black text-black' } ml-4 border-2 w-[94.5%] h-[20%] rounded-md text-center text-[20px] font-medium`} onDragStart={(e)=>{e.preventDefault();}} onDrop={(e)=>{e.preventDefault();}} onCut={(e)=>{e.preventDefault();}} onPaste={(e)=>{e.preventDefault();}} onCopy={(e)=>{e.preventDefault();}} />
        ...
        <>
            ...
            <section className={`select-none w-[500px] h-[250px] duration-400 rounded-md border-2 ${DarkMode ? 'border-white':'border-black'}`}>
                ...
                <form onSubmit={HandleSubmit} className={`${DarkMode ? 'text-white':'text-black'} w-full h-[calc(100%-18%)] flex flex-col gap-6 rounded-b-md px-6 pt-6`}>
                    <label htmlFor="projectID" className={`text-[18px] duration-400 font-extrabold quicksand-shit`}>Project ID:</label>
