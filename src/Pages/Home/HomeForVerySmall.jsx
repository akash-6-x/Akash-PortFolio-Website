  ...
  return (
    <>
        <section className="w-full flex flex-col items-center overflow-auto select-none">
                ...
                <aside className="w-[90%] h-[200px] mt-6 flex justify-center items-center gap-4 shrink-0">
                    <div title="Me" className={`border-2 border-blue-500 ${DarkMode?'shadow-[0_0_100px_10px_#00BFFF]':'shadow-[0_0_50px_14px_#00BFFF]'} w-[90px] h-[90px] rounded-full shrink-0`}>
                    ...
                    <div className={`${DarkMode?'text-white':'text-black'} text-[14px] cinzel-shit border  w-30 h-10 rounded-[6px] flex justify-center items-center shrink-0`}>Hey I'm Akash</div>
                </aside>

                <aside className="w-[90%] h-[600px] mb-6 flex flex-col justify-center items-center shrink-0">
                    <div className="w-[90%] h-[60%] mt-6 px-4 flex justify-center items-center">
                        <p className={`leading-relaxed ${DarkMode?'text-white':'text-black'} border tektur-shit text-[10px] p-6 rounded-[14px]`}>
                    ...
                    <div className="w-[90%] h-[10%] mt-16 flex justify-center items-center shrink-0 will-change-transform">
                        <div className="w-[200px] h-[35px] bg-blue-300 flex justify-evenly items-center rounded-[16px]">
                            ...
                            <div tabIndex={0} title="Go to Linked In Profile" aria-label="Go to Linked In Profile" className={`bg-blue-600 ${isLinkedInFocused && 'shadow-[0_0_20px_4px_blue]'} ${DarkMode?'border-white':'border-black'} w-[25px] h-[25px] border rounded-full flex justify-center items-center ease-in-out duration-400`} onFocus={()=>setisLinkedInFocused(true)} onBlur={()=>setisLinkedInFocused(false)} onClick={LinkedIn} onKeyDown={(e)=>{if(e.key === 'enter' || e.key === ' '){LinkedIn();}}}>
                                <LuLinkedin className="text-[11px] stroke-white fill-white"/>
                            </div>
                            <div tabIndex={0} title="Go to Instagram Profile" aria-label="Go to Instagram Profile" className={`bg-pink-500 ${isInstagramFocused && 'shadow-[0_0_20px_4px_blue]'} ${DarkMode?'border-white':'border-black'} w-[25px] h-[25px] border rounded-full flex justify-center items-center ease-in-out duration-400`} onFocus={()=>setisInstagramFocused(true)} onBlur={()=>setisInstagramFocused(false)} onClick={Instagram} onKeyDown={(e)=>{if(e.key === 'enter' || e.key === ' '){Instagram();}}}>
                                <LuInstagram className='text-[14px] stroke-white'/>
                            </div>
                            <div tabIndex={0} title="Go to Skills & Projects Page" aria-label="Go to Projects Page" className={`bg-green-600 ${isProjectsFocused && 'shadow-[0_0_20px_4px_blue]'} ${DarkMode?'border-white':'border-black'} w-[25px] h-[25px] border rounded-full flex justify-center items-center ease-in-out duration-400`} onFocus={()=>setisProjectsFocused(true)} onBlur={()=>setisProjectsFocused(false)} onClick={(e)=>{e.currentTarget.blur();Projects()}} onKeyDown={(e)=>{if(e.key === 'enter' || e.key === ' '){e.currentTarget.blur();Projects();}}}>
                                <LuCode className="text-[12px] stroke-white stroke-3"/>
                            </div>
                        </div>
                    </div>
                </aside>
            <div className="select-none mb-2 text-gray-600 text-[10px]">&copy; Copyright 2025. All Rights Reserved.</div>
        </section>
...
export default HomeForVerySmall